...
exports.run = async () => {
  ...
  for (const command of ['mesh.open', 'mesh.refresh', 'mesh.configureServer', 'mesh.configureStartup', 'mesh.checkUpdates', 'mesh.updateNow']) assert(commands.includes(command));
  const ready = async () => {
    for (let i = 0; i < 300; i++) {
