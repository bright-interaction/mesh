...
why: 'The collaboration goal: minimal-config onboarding, vaults sync with each other, local-first like Obsidian, plus an online option where an AI catches each contributor''s updates and compiles/structures them while pushing a Mesh update and reconciling concurrent changes. Mesh-native hub fits single-binary + sovereignty + minimal footprint (no second daemon); reconcile-first tames the spec''s riskiest subsystem (reliable delivery); the zero-model core means only markdown syncs, so the embedding-homogeneity-at-join problem evaporates. The sync-curator is the flywheel at team scale.'
status: accepted
---
