...
func TestSafeSyncPath(t *testing.T) {
	cases := []struct {
		...
		{name: "maximum component", path: "notes/" + strings.Repeat("a", 252) + ".md", wantOK: true},
		{name: "oversized filename", path: "notes/" + strings.Repeat("a", 253) + ".md", wantOK: false},
		{name: "oversized directory", path: strings.Repeat("a", 256) + "/a.md", wantOK: false},
		{name: "oversized unicode bytes", path: strings.Repeat("界", 85) + ".md", wantOK: false},
